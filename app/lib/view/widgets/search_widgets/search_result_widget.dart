...
final searchResult = FutureProvider<List<Player?>>((ref) async =>
    await PlayersDatabaseManager().searchForPlayer(ref.watch(searchInput)));
      ...
      children: [
        ...
        Expanded(child: Consumer(
          builder: (BuildContext context, ref, Widget? child) {
            ...
            return d.when(
                data: (val) => val.isNotEmpty
                    ? ListView.builder(
                        itemCount: val.length,
                        itemBuilder: (context, index) => FluentTheme(
                              data: FluentThemeData.dark(),
                              child: PlayerNameWithImage(
                                playerName: val[index]!.playerName,
                                playerId: val[index]!.playerId,
                                imagePath: val[index]!.imagePath,
                                playerIndexId: val[index]!.playerIndexId,
                              ),
                            ))
                    : Center(
                        child: Column(
