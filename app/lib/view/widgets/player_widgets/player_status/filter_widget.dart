...
class FilterNotifier extends StateNotifier<Map<String, dynamic>> {
  FilterNotifier()
      : super({
          ...
          'duration': {
            'id': DurationTime(
                begDate: DateTime.now().subtract(const Duration(days: 1)),
  ...
  void setCoachId(CustomBoxData coach) {
    state = {
      ...state,
      'coach': {"id": coach.id, 'title': coach.title},
    };
  }
  ...
  void setDuration(DurationModel val) {
    state = {
      ...state,
      'duration': {'id': val.value, "title": val.title}
    };
  }
  ...
  void setGender(CustomBoxData gender) {
    state = {
      ...state,
      'gender': {"id": gender.id, "title": gender.title}
    };
  }
  ...
  void setSubscription(CustomBoxData sub) {
    state = {
      ...state,
      "subscription": {'id': sub.id, 'title': sub.title}
    };
  }
}

final filterNewProvider =
    StateNotifierProvider<FilterNotifier, Map<String, dynamic>>(
  (ref) => FilterNotifier(),
);
final emptyProvider = StateProvider((ref) => null);
final reSubscriptionProvider = StateProvider<String>(
  (ref) => "0",
);

final filterEndedProvider =
    StateNotifierProvider<FilterNotifier, Map<String, dynamic>>(
  (ref) => FilterNotifier(),
);
              ...
              children: [
                FilterElement(
                  future: SubscriptionInformationManager()
                      .getAllSubscriptionsOrdered(),
                  title: 'Subscription value',
                  list: null,
                  allButton: true,
                  isSubscription: true,
                  onChange: (val) {
                    setFilter.setSubscription(val == "null"
                        ? CustomBoxData(title: "All", id: null)
                        : CustomBoxData(title: val.title, id: val.id));
                  },
                  provider: provider,
                  isSubscriptionWithName: false,
                ),
                Consumer(
                  builder:
                    ...
                    return Text(
                        filterValues['subscription']['title'].toString());
                  },
                )
              ],
            );
            ...
            return Row(
              ...
              children: [
                FilterElement(
                  ...
                  isSubscription: false,
                  onChange: (val) {
                    setFilter.setDuration(val.id != "null"
                        ? DurationModel(
                            title: val.title,
                            value: DurationTime(
                                begDate: val.id.begDate,
                                endDate: val.id.endDate))
                        : DurationModel(
                            title: "Today",
                            value: DurationTime(
