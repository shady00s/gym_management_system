...
class CalenderChartModel{
  ...
  CalenderChartModel( {required this.monthName,required this.height, required this.revenueValue});
}
class CalenderModel{
...
List<CalenderChartModel> setChartBarHeight(List<int> profitList){
  ...
  for (int idx =0; idx < transformedProfitList.length; idx++) {
    ...
    double height = ((transformedProfitList[idx] - lowestTransformedProfit) /
      ...
      chartBarHeightList.add(CalenderChartModel(monthName: months[idx],revenueValue:profitList[idx], height:profitList[idx] != 0 && profitList[idx] == lowestProfit ?lowestHeightFor: height));
...
List<int> setLeftSideChartData(List<int> data){
 ...
 var chartProfitProvider = FutureProvider((ref) => getProfit(ref.watch(yearProvider)));
Future <CalenderModel> getProfit(int year) async{
  ...
  await  GymStatusManager().getBestMonthInProfit(DateTime(year,6,1),DateTime(year,6,30)).then((value) => profit.insert(5,value??0));
  await  GymStatusManager().getBestMonthInProfit(DateTime(year,7,1),DateTime(year,7,31)).then((value) => profit.insert(6,value??0));
  await  GymStatusManager().getBestMonthInProfit(DateTime(year,8,1),DateTime(year,8,31)).then((value) => profit.insert(7,value??0));
          ...
          Center(
            child: Consumer(
                ...
                builder: (context, ref,child) {
                 ...
                 return futureData.when(data: (snapshot){
                          ...
                          Positioned(
                            ...
                            child: Padding(
                              ...
                              child:  SizedBox(
                                ...
                                width: 45,
                                child:DecoratedBox(
                                  decoration:const BoxDecoration(border: Border(right: BorderSide(color: Color.fromRGBO(
                                      45, 45, 45, 1.0)))),
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      ...snapshot.leftSide.map((e)=>Text(e.toString())).toList(),]),
                                ),
                              ),
                            ),
                          ),
                                ...
                                builder: (BuildContext context, BoxConstraints constraints) {
                                  return  SizedBox(
                                      ...
                                      child:Row(
                                          ...
                                          children:
                                          snapshot.chart.map((e) {
                                            int idx = snapshot.chart.indexOf(e);
                                            return Column(
                                              ...
                                              children: [
                                                MouseRegion(
                                                  onHover: (event){
                                                    RenderBox box =  e.keys[idx].currentContext!.findRenderObject() as RenderBox;
                                                    final position = box.localToGlobal(Offset.zero);
                                                    setState(() {

                                                      topPosition = e.height < 220? e.height +40 : e.height -10;
                                                      currentRevenue = e.revenueValue;
                                                      currentMonth = e.monthName;

                                                      leftProfitPosition = position.dx -(MediaQuery.sizeOf(context).width * 0.64);
                                                  ...
                                                  child: LayoutBuilder(
                                                    ...
                                                    builder: (BuildContext context, BoxConstraints constraints) {
                                                      return Card(
                                                        ...
                                                        ,padding: EdgeInsets.zero, backgroundColor: Colors.green, child:  SizedBox(height: e.height+2,width: 10,),);
                                                    },
